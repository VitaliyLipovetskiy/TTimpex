...
public class DepartmentDeserializer extends StdDeserializer<Department> {
    ...
    @Override
    public Department deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JacksonException {
        JsonNode node = jsonParser.getCodec().readTree(jsonParser);

        Department feedback = new Department();
        feedback.setId(node.get("id").asInt());
